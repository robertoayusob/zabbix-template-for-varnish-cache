...
7. Remember that "-s dev" must be the hostname set on the Zabbix server so if you use ``HostnameItem=system.hostname`` on zabbix_agentd.conf you have to use ``-s $(/usr/bin/hostname)`` (centos based).
